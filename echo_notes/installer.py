...
def get_script_dir():
    ...
    # This ensures the installer works correctly when run from a temporary directory
    return Path.cwd()
...
def setup_venv(install_dir):
            ...
            for executable in ["python", "python3", "pip", "pip3"]:
                ...
                if exec_path.exists():
                    try:
                        ...
                        print_color(Colors.YELLOW, f"Warning: Could not set executable permissions for {exec_path}")
    ...
    if (install_dir / "setup.py").exists() or (install_dir / "pyproject.toml").exists():
        try:
            ...
            print_color(Colors.YELLOW, f"Warning: Could not install package in development mode: {e}")
    else:
        print_color(Colors.YELLOW, "Skipping package installation (no setup.py or pyproject.toml found)")
...
def create_linux_shortcut(install_dir, venv_dir):
    ...
    # Look for the icon file in different possible locations
    icon_paths = [
    ...
    for icon_path in icon_paths:
        if icon_path.exists():
            ...
            print_color(Colors.GREEN, f"Installed icon from {icon_path}")
            icon_found = True
            break
    
    if not icon_found:
        print_color(Colors.YELLOW, "Warning: Could not find icon file. Skipping icon installation.")
    ...
    try:
        dashboard_symlink.symlink_to(install_dir / "echo_notes/dashboard.py")
        os.chmod(install_dir / "echo_notes/dashboard.py", 0o755)
    except (PermissionError, OSError) as e:
        print_color(Colors.YELLOW, f"Warning: Could not create symlink: {e}")
...
Exec={venv_dir}/bin/python {install_dir}/echo_notes/dashboard.py
...
def create_macos_shortcut(install_dir, venv_dir):
    ...
    for icon_path in icon_paths:
        if icon_path.exists():
            shutil.copy(icon_path, app_dir / "Contents/Resources/echo-notes.png")
            print_color(Colors.GREEN, f"Installed icon from {icon_path}")
            icon_found = True
            break
...
""")
    ...
    with open(app_dir / "Contents/MacOS/echo-notes-launcher", "w") as f:
        f.write(f"""#!/bin/bash
# Echo Notes Launcher
"{venv_dir}/bin/python" "{install_dir}/echo_notes/dashboard.py"
""")
    ...
    try:
        ...
        print_color(Colors.YELLOW, f"Warning: Could not set executable permissions: {e}")
    ...
    try:
        ...
        if usr_local_bin.exists() and os.access(usr_local_bin, os.W_OK):
            ...
            if symlink_path.exists() or symlink_path.is_symlink():
                symlink_path.unlink()
            symlink_path.symlink_to(install_dir / "echo_notes/dashboard.py")
            os.chmod(install_dir / "echo_notes/dashboard.py", 0o755)
...
# Look for icon in different locations
$IconPath = $null
$IconPaths = @(
    '{install_dir / "config/icons/Echo-Notes-Icon.png"}',
    '{install_dir / "Echo-Notes-Icon.png"}',
...
def configure_app(install_dir):
    ...
    config_file = install_dir / "echo_notes/shared/schedule_config.json"
...
""")
        ...
        print_color(Colors.YELLOW, f"Warning: Could not create configuration file: {e}")
...
def create_uninstaller(install_dir, venv_dir):
    ...
    if os_type == "windows":
        uninstaller_path = install_dir / "installers/windows/uninstall.bat"
...
""")
        ...
        uninstaller_path = install_dir / "installers/uninstall.sh"
        # Create directory if it doesn't exist
        uninstaller_path.parent.mkdir(parents=True, exist_ok=True)
        with open(uninstaller_path, "w") as f:
            f.write(f"""#!/bin/bash
...
def main():
    ...
    parser.add_argument("--uninstall", action="store_true", help="Uninstall Echo-Notes")
    parser.add_argument("--keep-config", action="store_true", help="Keep configuration files when uninstalling")
    parser.add_argument("--purge", action="store_true", help="Remove everything including notes when uninstalling")
    ...
    if args.uninstall:
        ...
        if os_type == "linux":
            ...
            uninstall_linux(install_dir, {"purge": args.purge, "keep_config": args.keep_config})
        elif os_type == "macos":
            from installers.macos.macos_uninstaller import uninstall_macos
            uninstall_macos(install_dir, {"purge": args.purge, "keep_config": args.keep_config})
        elif os_type == "windows":
            from installers.windows.windows_uninstaller import uninstall_windows
            uninstall_windows(install_dir, {"purge": args.purge, "keep_config": args.keep_config})
        else:
            print_color(Colors.YELLOW, "Unknown operating system. Performing generic uninstallation.")
            # Generic uninstallation
            remove_venv(install_dir)
            if args.purge:
                notes_dir = os.environ.get("ECHO_NOTES_DIR", str(Path.home() / "Documents/notes/log"))
    ...
    if detect_os() == "windows":
        print("To uninstall, run uninstall.bat in the Echo-Notes/installers/windows directory")
    else:
        print("To uninstall, run ./uninstall.sh in the Echo-Notes/installers directory")
    print("")
